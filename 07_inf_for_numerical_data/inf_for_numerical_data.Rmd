...
Open the `Distributions` menu (click the plus sign and check the box if you don't see the menu at the top), and select `Student's t`. For degrees of freedom, we need to enter 2 less than the total number of valid observations. Since some individuals are missing values, we can't just use the total number of observations. Use a discriptives table with both `weight` and `physical_3plus` and determine the total number of valid obervations, then subtract 2 from that value. Enter this in the `Degree of freedom` box in the `Student's t` menu.
...
In the options, click to highlight `Probability`, the select "from $-\infty$ to 0" and replace 0 with your observed difference. You will see a (rounded) value show up, and the area will be highlighted (if enough is visible to be highlighted). (This is actually half of the p-value, since we did not specify a direction, and so are doing a t-tailed test.)

While we were able to determine an approximate p-value using this package, JASP will let us calculate a more precise value directly. Open the `T-Tests` menu, then select `Independent Samples T-Test`. Select `weight` as your variable and `physical_3plus` as your `Grouping Variable`. This will create a table on the right hand side.

5. What is the value in the `t` column. Where else did this number show up?

6. What is the p-value? How does this compare to the value from the t-distribution graph?

Next we can add a confidence interval to our table. Check the box labeled `Location parameter` below `Additional Statistics`, and then also check `Confidence interval` and leave the value at 95.0. You will now see the values for the confidence interval in your table.
