...
The name of the data frame is `us_adults` and the name of the variable that contains responses to the question *"Do you think climate change is affecting your local community?"* is `climate_change_affects`. The file can be found [here](https://github.com/OpenIntroStat/oilabs-jasp/raw/main/05b_confidence_intervals/more/us_adults.csv).

We can quickly visualize the distribution of these responses using a bar plot. Do this using the `Descriptives` menu.
...
In this lab, you'll start with a simple random sample of size 60 from the population. As in the last lab, we will use the binomial section in the distributions menu to do this. Since 62% responded "yes" to the question, we will use 0.62 as the probability of success. For the number of trials, enter 60. In the `Generate and Display Data` section, enter the variable name as `samp` and 1 as the number of samples.
...
In a `Binomial` analysis, we will continue to use a probability of success of 0.62, number of trials 60, but now use 1000 as our number of samples, and name this variable `bootstrap`. Draw the samples.

Since we want to estimate the population proportion, we'll create one more variable, `bootstrap_perc` which will be `bootstrap` divided by 60.

Now, open a descriptives analysis and add the `bootstrap_perc` variable. Open the `Statistics` submenu, and check the `Percentiles` box. Since we want a 95% confidence interval, we want to cut at the 2.5 and 97.5 percentiles, so enter `2.5, 97.5` in the box. You will see these values show up in the descriptive statistics table to the right.
...
4.  Lastly, try one more (different) confidence level.
    ...
    Then, calculate the bounds of the interval using JASP and data from `samp` and interpret it.
