...
Create this new variable, and call it `text_ind` and use the `ifElse()` function, where the values are either `yes` or `no`. We'll see that this isn't strictly necessary, but it makes some things easier to see.
...
Click `Frequencies`, then `Binomial Test` in the `Classical` section. Put `text_ind` in the Variables box, and check the box `Confidence interval` in the `Additional Statistics` section. Take a look at the results to the right of your screen. You should see a table of results - we will focus on the section under the heading `95% CI for Proportion`. You should see a `Lower` and `Upper` value for each of `no` and `yes`. These values give the upper and lower bounds for the 95% confidence interval for each value of `text_ind`.
...
Since sample size is irrelevant to this discussion, let's just set it to some value ($n = 1000$) and use this value in our calculations. In the dataset [here](https://github.com/OpenIntroStat/oilabs-jasp/raw/main/06_inf_for_categorical_data/more/me.csv), there are two columns. The first is a variable `p` that is a sequence from 0 to 1 with each number incremented by 0.01.
