...
Here we'll use the graphical tools of JASP to assess the normality of our data and also learn how to generate random numbers from a normal distribution.
...
We can answer this by simulating data from a normal distribution. Make a note of the mean and standard deviation from our `cal_fat` variable - we're going to need them. Still within the Normal distributions menu, we can now click the `Show Distribution` menu. We're going to specify the mean and standard deviation for our distribution, so in the `Parameters` pulldown menu, select $\mu, \sigma$, and in the boxes below, type the mean and standard deviation of our variable in.
...
We now have `sim_norm` as a variable that we can work with, so we can inspect it. 
