...
The name of the data frame is `global_monitor` and the name of the variable that contains responses to the question *"Do you believe that the work scientists do benefit people like you?"* is `scientist_work`. The data is [here](https://github.com/OpenIntroStat/oilabs-jasp/raw/main/05a_sampling_distributions/more/global_monitor.csv). 

We can quickly visualize the distribution of these responses using a bar plot. You can find this in the `Descriptives` menu, by clicking `Distribution plots` under `Basic plots`.
...
If you are interested in estimating the proportion of people who don't think the work scientists do benefits them, you can use the `Distributions` tool to survey the population.
...
Here, we use JASP to take 15,000 different samples of size 50 from the population, calculate the proportion of responses in each sample, and store the results.
...
1.  To make sure you understand how sampling distributions are built, and exactly what the process does, try modifying the process to create a sampling distribution of **25 sample proportions** from **samples of size 10**. How many observations are there in this object? What does each observation represent?
...
4.  Of the sampling distributions from 9 and 10, which has a smaller spread?
